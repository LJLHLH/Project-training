...
router.post('/add',(req,res)=>{
  ...
  //执行SQL语句
  pool.query('INSERT INTO xiaomi_commodity SET ?',[obj],(err,result)=>{
    if(err) throw err;
	//console.log(result);
	//如果数据插入成功，响应对象
...
//建立连接池
pool.query('DELETE * FROM xiaomi_commodity WHERE lid=?',[obj.lid],(err,result)=>{
  if(err) throw err;
  if(result.affectedRows>0){
...
router.get("/list",(req,res)=>{
  ...
  //如果页码为空，默认是1；如果大小为空，默认为2
	...
	//执行SQL语句
	let sql = `SELECT count(*) as Count FROM xiaomi_commodity;SELECT lid,title,price,sold_count,is_onsale,md as pic FROM xiaomi_commodity xiaomi_commodity INNER JOIN (select makeup_id,md from xiaomi_makeup_pic group by makeup_id) xiaomi_makeup_pic ON xiaomi_commodity.lid = xiaomi_makeup_pic.makeup_id LIMIT ?,?;`;
...
router.get("/detail", (req, res) => {  
 ...
 let sql = `SELECT * FROM xiaomi_commodity WHERE lid=?;
               SELECT pid,makeup_id,sm,md,lg FROM xiaomi_makeup_pic WHERE makeup_id=?;//图片
               SELECT fid,fname FROM xiaomi_commodity_type INNER JOIN xiaomi_commodity ON xiaomi_commodity_type.fid = xiaomi_commodity.family_id WHERE xiaomi_commodity.lid = ?;//详情
               SELECT lid,spec FROM xiaomi_commodity WHERE family_id IN (SELECT family_id FROM xiaomi_commodity WHERE lid = ?); `;
  pool.query(
    sql,
