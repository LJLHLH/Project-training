...
router.post('/reg',(req,res)=>{
   ...
   //1.3执行SQL语句
   pool.query('INSERT INTO xiaomi_user SET ?',[obj],(err,result)=>{
       if(err) throw err;
       console.log(result);
       //如果数据插入成功 响应对象
 ...
 router.post('/login',(req,res)=>{
  ...
  //2.3执行SQL语句
  pool.query('SELECT * FROM xiaomi_user WHERE uname=? AND upwd=?',[obj.uname,obj.upwd],(err,result)=>{
    if(err) throw err;
    //console.log(result);
...
router.get('/detail',(req,res)=>{
 ...
 //3.3 执行SQL语句
 pool.query('SELECT uid,uname,birthday,upwd,email,phone,gender,user_name FROM xiaomi_user WHERE uid=?',[obj.uid],(err,result)=>{
   if(err) throw err;
   //console.log(result);
   if(result.length>0){
     res.send({
...
router.get('/delete',(req,res)=>{
  ...
  //4.3执行SQL语句
  pool.query('DELETE FROM xiaomi_user WHERE uid=?',[obj.uid],(err,result)=>{
     if(err)throw err;
  //console.log(result);
  if(result.affectedRows>0){
...
router.get('/update',(req,res)=>{
  ...
  //5.3执行SQL语句
  pool.query('UPDATE xiaomi_user SET ? WHERE uid=?',[obj,obj.uid],(err,result)=>{
    if(err)throw err;
    //console.log(result);
    if(result.affectedRows>0){
...
router.get('/list',(req,res)=>{
  ...
  //6.5执行SQL语句
  pool.query('SELECT * FROM xiaomi_user LIMIT ?,?',[start,obj.count],(err,result)=>{
    if(err)throw err;
...
router.get('/checkemail',(req,res)=>{
  ...
  //7.3执行SQL语句
  pool.query('SELECT * FROM xiaomi_user WHERE email=?',[obj.email],(err,result)=>{
    if(err) throw err;
  //console.log(result);
  if(result.length>0){
...
if(!obj.phone){
 ...
 pool.query('SELECT * FROM xiaomi_user WHERE phone= ?',[obj.phone],(err,result)=>{
  if(err)throw err;
  //console.log(result);
  if(result.length>0){
...
if(!obj.uname){
 ...
 pool.query('SELECT * FROM xiaomi_user WHERE uname= ?',[obj.uname],(err,result)=>{
  if(err)throw err;
  if(result.length>0){
...
router.get('/sessiondata',(req,res)=>{
  ...
  pool.query('SELECT * FROM xiaomi_user WHERE uid=?',[obj.uid],(err,result)=>{
    if(err) throw err;
    if(result.length>0){
