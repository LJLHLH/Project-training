...
router.post("/add", (req, res) => {
    ...
    //判断用户的购物车中是否已有该商品
    let sql = `SELECT iid FROM xiaomi_shoppingcart_item WHERE user_id=? AND product_id=?`;
    pool.query(sql, [uid, lid], (err, result) => {
        if (err) throw err;
        let sql2;
        if (result.length > 0) {
            //用户购物车已经有该商品，则加1
            sql2 = `UPDATE xiaomi_shoppingcart_item SET count=count+${buyCount} WHERE user_id=${uid} AND product_id=${lid}`;
        } else {
            sql2 = `INSERT INTO xiaomi_shoppingcart_item VALUES(NULL, ${uid}, ${lid}, ${buyCount}, false)`;
        }
...
router.get("/list", (req, res) => {
    ...
    let sql = `SELECT a.iid,a.product_id as lid ,b.title,b.spec, b.price,a.count,'' as pic 
                  FROM xiaomi_shoppingcart_item a INNER JOIN xiaomi_laptop b ON a.product_id = b.lid 
                  WHERE a.user_id = ? `;
    pool.query(sql, [user.id], (err, result) => {
      ...
      for (let i = 0; i < retJson.data.length; i++) {
        ...
        (function (lid, i) {
          pool.query(
            "SELECT md FROM xiaomi_makeup_pic WHERE makeup_id=? LIMIT 0,1",
            [lid],
            (err, result) => {
...
router.get("/delete", (req, res) => {
    ...
    //1.3执行SQL语句
    let sql = "DELETE FROM xiaomi_shoppingcart_item WHERE iid =?";
...
router.get("/updatecount", (req, res) => {  
    ...
    //1.3执行SQL语句
    let sql = "UPDATE xiaomi_shoppingcart_item SET count = ? WHERE iid=?";
...
router.get("/updatechecked", (req, res) => {
    ...
    let sql = "UPDATE xiaomi_shoppingcart_item SET is_checked=? WHERE iid=?";
